...
export function buildAdminWhatsAppUrl({ name, phone, serviceLabel, styleLabel, date, time, totalPrice, depositAmount }) {
  const message = `حجز جديد في Rezo Lash%0A%0Aالاسم: ${name || "-"}%0Aالجوال: ${phone || "-"}%0Aالخدمة: ${serviceLabel || "-"}%0Aالرسمة: ${styleLabel || "-"}%0Aالتاريخ: ${date || "-"}%0Aالوقت: ${time || "-"}%0Aالإجمالي: ${totalPrice || 0} SAR%0Aالعربون: ${depositAmount || 0} SAR`;
...
export function isPastDateTime(date, time) { const selected = new Date(`${date}T${time}:00`); return selected.getTime() < Date.now(); }
